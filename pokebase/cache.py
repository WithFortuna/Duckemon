...
import os
import shelve, portalocker
import time

from pokebase.common import cache_uri_build, sprite_filepath_build
...
def save(data, endpoint, resource_id=None, subresource=None):
    ...
    while True:
        try:
            with portalocker.Lock(API_CACHE + '.lock', timeout=100): #추가
                with shelve.open(API_CACHE) as cache:               #추가
                    cache[uri] = data
            break
        except portalocker.exceptions.AlreadyLocked:
            print(f"파일 잠겨서 대기중... 위치: cache.save")
            time.sleep(1)

        except OSError as error:
            if error.errno == 11:  # Cache open by another person/program
                print('cache가 다른 프로세스에서 열려있음. 재시도함')
                time.sleep(1)
                # pass
            else:
                raise error
...
def load(endpoint, resource_id=None, subresource=None):
    ...
    while True:
        try:
            with portalocker.Lock(API_CACHE+'.lock', timeout=90 ):  #추가
...
# CACHE_DIR, API_CACHE, SPRITE_CACHE = set_cache('C:/Users/NT551XCJ/Desktop/start_backend/Project/Duckemon/cacheFile/')

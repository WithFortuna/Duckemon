...
def sprite_url_build(sprite_type, sprite_id, **kwargs): #sprite_type: pokemon, item..
    ...
    if sprite_type == "pokemon":
        if kwargs.get("model", False):
            ...
            if kwargs.get("official_artwork", False):
                options.append("official-artwork")  ##"official-artwork/shiny"는 포함이 안되는거 아닌가???
                # if kwargs.get("shiny",False): #edited by WithFortuna
                #     options.append("shiny")  #edited by WithFortuna
